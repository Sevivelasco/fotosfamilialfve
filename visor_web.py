...
# Usamos st.session_state para almacenar variables
if 'menu_state' not in st.session_state: st.session_state.menu_state = 'INICIO'
if 'df_cache' not in st.session_state: st.session_state.df_cache = {} # Caché para los DataFrames individuales
...
def update_index(direction):
    """
    Cambia la foto actual (Izquierda/Derecha) con navegación circular.
    Si llega al final, vuelve al principio. Si retrocede desde el principio, va al final.
    """
    ...
    if new_index >= total:
        st.session_state.photo_index = 0 # Vuelve a la primera foto
    elif new_index < 0:
        st.session_state.photo_index = total - 1 # Va a la última foto
    else:
        st.session_state.photo_index = new_index
        
    st.rerun() # Re-ejecutar la app para mostrar la nueva foto
        ...
        for key, config in consultas_individuales.items():
            if st.button(f"{key} - {config['nombre']}", key=f"btn_{key}"):
                ...
                if not df_temp.empty:
                    # ⚠️ Lógica crítica: Crear las columnas de la ruta y la foto
